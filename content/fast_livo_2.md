...
1. Employ a sequetntial update strategy (first running LIO then running VIO based on the state updated by LIO) to effectively fuse heterogenous measurements from LiDAR and camera sensors into a unified system.
...
$$ \begin{align} I : \text{IMU frame} \\ G : \text{Global frame} \end{align} $$
